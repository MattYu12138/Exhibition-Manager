...
function initSchema() {
  ...
  if (systems.cnt === 0) {
    db.prepare(`
      ...
      VALUES
        ('SYS001', 'exhibition-manager', 'Exhibition Manager', '展会管理系统，管理展会商品、库存快照和 Square 集成', '/exhibition', '🎪', 1),
        ('SYS002', 'inventory-manager', 'Inventory Manager', '库存管理系统，检测重复 Barcode/SKU，同步 Shopify 商品数据', '/inventory', '📦', 2)
    `).run();
